...
*      INTO TABLE @DATA(keys).
    ...
    SELECT
      FROM /lrn/passflight
    FIELDS carrier_id, connection_id, flight_date,
"2.3 añadir campo seats_max - seats_occupied AS seats_free:
           plane_type_id, seats_max, seats_occupied, seats_max - seats_occupied AS seats_free,
"
           price, currency_code
     WHERE carrier_id = @i_carrier_id
...
"2.4 añadir seats_max - seats_occupied AS seats_free a la lista de campos:
...
"2.5 sustituir la resta por solo flight_raw-seats_occupied:
*      seats_free = flight_raw-seats_max - flight_raw-seats_occupied.
      seats_free = flight_raw-seats_occupied.
"
...
"3.3 sustituir name por un string con la concatenacion de nombre e id:
...
"3.2 comentar la concatenación del atributo
